...
def get_dxf(path_file) -> rvec.ListVector:
    return isoreader.iso_read_continuous_flow(path_file)

def convert(data: rvec.ListVector) -> dict[str, dict | pd.DataFrame | np.ndarray]:
    ...
    for k, e in data.items():
        if isinstance(e, rvec.DataFrame):
            df = pd.DataFrame(e).T
            df.columns = e.colnames
            df.index = e.rownames
            objs[k] = df
        elif isinstance(e, rvec.StrVector | rvec.IntVector | rvec.FloatVector | rvec.BoolVector):
            vec = np.array(e)
            if len(vec) == 1:
                vec = vec[0]
            objs[k] = vec
        elif isinstance(e, rvec.ListVector):  # start recursion
            objs[k] = convert(e)
        else:
...
class DXF:
    def __init__(self, path_file: str):
        ...
        data: rvec.ListVector = isoreader.iso_read_continuous_flow(path_file)
